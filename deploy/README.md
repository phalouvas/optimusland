...
docker compose --project-name traefik \
  --env-file ~/optimusland/deploy/traefik.env \
...
```shell
docker compose --project-name mariadb --env-file ~/optimusland/deploy/mariadb.env -f ~/frappe_docker/overrides/compose.mariadb-shared.yaml up -d
```

**phpMyAdmin**

```shell
docker compose --project-name phpmyadmin --env-file ~/optimusland/deploy/phpmyadmin.env -f ~/frappe_docker/overrides/compose.phpmyadmin.yaml up -d
```

**Erpnext**

Create a yaml file called `erpnext-v15.yaml` in `~/optimusland/deploy` directory:

```shell
docker compose --project-name erpnext-v15 \
  --env-file ~/optimusland/deploy/erpnext-v15.env \
  ...
  -f ~/frappe_docker/overrides/compose.multi-bench-ssl.yaml config > ~/optimusland/deploy/erpnext-v15.yaml
```

```shell
docker compose --project-name erpnext-v15 -f ~/optimusland/deploy/erpnext-v15.yaml up -d
```
...
- Export apps to variable and build image
  ...
  export APPS_JSON_BASE64=$(base64 -w 0 ~/optimusland/deploy/v15.json)
...
```shell
~/optimusland/deploy/deploy.sh
```

To delete old image in order to free up space use command `docker rmi -f phalouvas/optimusland-worker:x.x.x` where x.x.x the old version
